...
export default function MenuItem({ item }: { item: MenuItem }) {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-between gap-4 bg-[#111] p-4">
      <Link
        className="flex w-full flex-row justify-between gap-4"
        href={`/menu/${item._id}`}
      >
        <div>
          <p className="text-menusecondary text-lg font-bold">
            {item.name}{" "}
            {item.isVeg && (
          ...
          <p className="text-itemdescription line-clamp-2 text-sm">
            {item.description}
          </p>
        </div>
        <div>
      ...
      <div className="flex w-full flex-row items-center justify-between gap-4">
        <Link
          className="bg-menusecondary-foreground text-menuprimary rounded-3xl p-2 px-2 py-1 tracking-[1px]"
          href={`/menu/${item._id}`}
        >
          ...
          (cartItems.find((cart) => cart._idMenuItem === item._id)
            ...
            item && (
              <MenuChoosing item={item}>
                <Button
                  variant="secondary"
                  className="bg-menuprimary text-menuforeground hover:bg-buttonhover rounded-none px-8 text-lg font-bold"
                >
                  Add
                </Button>
              </MenuChoosing>
